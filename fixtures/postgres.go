...
func Postgres(db *sql.DB, location string, debug bool) *LoaderPostgres {
...
func (f *LoaderPostgres) loadYml(data []byte, ctx *loadContext) error {
	...
	for name, fields := range loadedFixture.Templates {
	...
	for tableName, sourceRows := range loadedFixture.Tables {
		rows := make(table, len(sourceRows))
		for i, sourceFields := range sourceRows {
